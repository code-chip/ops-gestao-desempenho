...
CONCAT(DATE_FORMAT('".$periodo."-01','%d/%m'),' a ".$date."') AS REGISTRO FROM DESEMPENHO AS D, (SELECT USUARIO_ID, AVG(DESEMPENHO) DESEMPENHO,ATIVIDADE_ID FROM DESEMPENHO WHERE ANO_MES='".$periodo."' AND PRESENCA_ID NOT IN (3,5) GROUP BY USUARIO_ID, ATIVIDADE_ID) AS B INNER JOIN USUARIO U ON U.ID=B.USUARIO_ID INNER JOIN ATIVIDADE A ON A.ID=B.ATIVIDADE_ID WHERE D.USUARIO_ID=B.USUARIO_ID AND ANO_MES='".$periodo."'".$meta." " .$turno." AND D.ATIVIDADE_ID=B.ATIVIDADE_ID ".$setor." GROUP BY D.USUARIO_ID, D.ATIVIDADE_ID ORDER BY ".$ordenacao.";";
	...
	$x = 0;
	$cnx = mysqli_query($phpmyadmin, "SELECT COUNT(1) AS QTD, (SELECT DATE_FORMAT(MAX(REGISTRO),'%d/%m') FROM DESEMPENHO WHERE CADASTRADO_POR = 58) AS REG, (SELECT COUNT(1) FROM USUARIO WHERE GESTOR_ID = 58 AND SITUACAO = 'Ativo') AS TIME FROM DESEMPENHO WHERE CADASTRADO_POR = 58 AND REGISTRO = (SELECT MAX(REGISTRO) FROM DESEMPENHO WHERE CADASTRADO_POR = 58) UNION SELECT COUNT(1) AS QTD, (SELECT DATE_FORMAT(MAX(REGISTRO),'%d/%m') FROM DESEMPENHO WHERE CADASTRADO_POR = 149) AS REG, (SELECT COUNT(1) FROM USUARIO WHERE GESTOR_ID = 149 AND SITUACAO = 'Ativo') AS TIME FROM DESEMPENHO WHERE CADASTRADO_POR = 149 AND REGISTRO = (SELECT MAX(REGISTRO) FROM DESEMPENHO WHERE CADASTRADO_POR = 149) UNION SELECT COUNT(1) AS QTD, (SELECT DATE_FORMAT(MAX(REGISTRO),'%d/%m') FROM DESEMPENHO WHERE CADASTRADO_POR = 31) AS REG, (SELECT COUNT(1) FROM USUARIO WHERE GESTOR_ID = 31 AND SITUACAO = 'Ativo') AS TIME FROM DESEMPENHO WHERE CADASTRADO_POR = 31 AND REGISTRO = (SELECT MAX(REGISTRO) FROM DESEMPENHO WHERE CADASTRADO_POR = 31) UNION SELECT COUNT(1) AS QTD, (SELECT DATE_FORMAT(MAX(REGISTRO),'%d/%m') FROM DESEMPENHO WHERE CADASTRADO_POR = 99) AS REG, (SELECT COUNT(1) FROM USUARIO WHERE GESTOR_ID = 99 AND SITUACAO = 'Ativo') AS TIME FROM DESEMPENHO WHERE CADASTRADO_POR = 99 AND REGISTRO = (SELECT MAX(REGISTRO) FROM DESEMPENHO WHERE CADASTRADO_POR = 99)
	");
	...
	<script type="text/javascript">
      ...
      function drawTable() {
        ...
        data.addRows([
          ...
          ['Leandro', parseFloat('<?php echo $a5time[1]?>'), parseFloat('<?php echo $a5count[1]?>'), {v: parseFloat('<?php echo $a5date[1]?>'),   f: '<?php echo $a5date[1]?>'},  <?php echo $a5status[1]?>],
          ['Karla', parseFloat('<?php echo $a5time[2]?>'), parseFloat('<?php echo $a5count[2]?>'), {v: parseFloat('<?php echo $a5date[2]?>'), f: '<?php echo $a5date[2]?>'}, <?php echo $a5status[2]?>],
