...
$cnx = mysqli_query($phpmyadmin, "SELECT 
  DATE_FORMAT(DATA_HORA, '%Y-%m-%d %H:%i') AS FORMATTED_DATA,
  TIMESTAMPDIFF(DAY, DATA_HORA, CURDATE()) AS DAYS,
  (SELECT COUNT(1) FROM FEEDBACK WHERE DESTINATARIO_ID = {$_SESSION['userId']} AND SITUACAO = 'Aprovado') AS FEEDBACK,
  (SELECT COUNT(1) FROM SOLICITACAO WHERE DESTINATARIO_ID = {$_SESSION['userId']} AND SITUACAO = 'Enviado') AS SOLICITACAO,
  (SELECT COUNT(1) FROM FEEDBACK WHERE SITUACAO = 'Enviado') AS WAIT,
  (SELECT MAX(ACESSO_TOTAL) FROM ACESSO WHERE USUARIO_ID = {$_SESSION['userId']}) AS ACESSOS
FROM ACESSO_HISTORICO 
WHERE USUARIO_ID = {$_SESSION['userId']}
