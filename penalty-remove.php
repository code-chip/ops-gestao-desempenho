...
<head>
	...
	<title>Gestão de Desempenho - Remover Penalidade</title>
...
<body>
	...
	<?php if (!isset($_POST['pesquisar'])) : ?>	
	  	<div class="container">
	  		<form action="penalty-remove.php" method="POST" id="form1">
	...
	if (isset($_POST['pesquisar'])) {
		...
		if (isset($_POST['pesquisar']) && mysqli_num_rows($cnx) == 0) {
			echo "<script>alert('Nenhum registrado encontrado com o filtro utilizado.'); window.location.href='penalty-remove.php';</script>";
		}
	...
	if ($x > 0) : { ?>
		<form action="penalty-remove.php" method="POST" onsubmit="return check()" id="form2">
		...
		if ($idType[$i] == $listIdType[$z]): { ?>
			...
			<div class="field">				
				<div class="control">
					<input name="penalty[]" style="max-width:5.5em;" type="text" class="input desempenho is-size-7-touch" placeholder="Obrigatório" maxlength="4" value="<?php echo $penalty[$i]; ?>" disabled>
				</div>				
			</div>
			</td>
			...
			<div class="field">				
				<div class="control">
					<input name="occurence[]" style="max-width:5.5em;" type="text" class="input desempenho is-size-7-touch" placeholder="Obrigatório" maxlength="4" value="<?php echo $occurence[$i]; ?>" disabled>
				</div>				
			</div>
			</td>
			<td><!--COLUNA DATA-->
			<div class="field">				
				<div class="control">
					<input name="register[]" style="max-width:6.5em;" type="text" class="input registro is-size-7-touch" value="<?php echo $register[$i];?>" maxlength="10" disabled>
				</div>				
			</div>
			</td>
			...
			<div class="field">				
				<div class="control">
					<input name="observation[]" type="text" class="input is-size-7-touch" placeholder="Máximo 200 caracteres." maxlength="200" value="<?php echo $observation[$i]; ?>" disabled>
				</div>				
			</div>
			</td></tr>						
			<?php 
		...
		<div class="field is-horizontal">
			...
			<div class="field-body">
				<div class="field is-grouped">							
					<div class="control">
						<button name="removePenalty" type="submit" class="button is-primary" value="remove">Remover</button>
					</div>
					<div class="control">
						<a href="penalty-remove.php" class="button is-primary">Voltar</a>
					</div>
					<div class="control">
						<a href="home.php" class="button is-primary">Cancelar</a>
					</div>						
...
if (isset($_POST["removePenalty"]) != null && isset($_POST["vetor"])) {
	$id = array_filter($_POST['vetor']);
	
	for ( $i = 0; $i < sizeof($id); $i++ ) {
		$rmv = "DELETE FROM PENALIDADE WHERE ID = " . $id[$i];
		$cnx = mysqli_query($phpmyadmin, $rmv);
	}
	
	if (mysqli_error($phpmyadmin) == null) {
		echo "<script>alert('Penalidade removida com sucesso!'); window.location.href=window.location.href; </script>";
	} else {
		echo "<script>alert('Erro ao remover penalidade!!'); window.location.href=window.location.href; </script>";
	}
} else if (isset($_POST["removePenalty"]) != null) {
